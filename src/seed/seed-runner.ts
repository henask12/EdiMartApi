...
import { CATALOG_CATEGORIES, CATALOG_PRODUCTS } from "../../prisma/catalog-seed-data";
...
export const runSeed = async (prisma: PrismaClient) => {
  ...
  await prisma.user.upsert({
    ...
    update: { passwordHash: await hash("Owner123!") },
    create: {
  ...
  await prisma.user.upsert({
    ...
    update: { passwordHash: await hash("Cashier123!") },
    create: {
  ...
  await prisma.user.upsert({
    ...
    update: { passwordHash: await hash("Staff123!") },
    create: {
